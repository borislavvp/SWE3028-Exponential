...
def predict(stockName):
    ...
    conv1_layer = tf.keras.layers.Conv2D(64, 7, strides=2, input_shape=[
                                         img_size, img_size, 3], padding='same', use_bias=False)(inputs_CNN)
    ...
    for i, filters in enumerate([64]*2 + [128]*4 + [256] * 6 + [512]*3):
        ...
        ResnetList.append(ResidualUnit(
            filters=filters, strides=strides)(ResnetList[i]))
        prev_filters = filters

    extractLayer = tf.keras.layers.Conv2D(512, 1, strides=1, input_shape=[
                                          87, 87, 512], padding='same', use_bias=False)(ResnetList[-1])
    ...
    def create_lstm_model(log_inputs: tf.keras.Input) -> layers.Layer:
        lstm_layer_1 = layers.LSTM(50, batch_input_shape=(
            None, 50, 1), return_sequences=True)(log_inputs)
        lstm_layer_2 = layers.LSTM(
            50, batch_input_shape=(None, 50, 50))(lstm_layer_1)
    ...
    model.load_weights('./%s.h5' % stockName)
    ...
    stock_ratio_up = (pred - 10**latest_stock_price +
    ...
    def gradCAM(orig, intensity=0.5, res=350):
        ...
        with tf.GradientTape() as tape:
            ...
            iterate = tf.keras.models.Model(
                lstm_cnn_model.input, [lstm_cnn_model.output, conv_layer])
        ...
        heatmap = tf.reduce_mean(tf.multiply(
            pooled_grads, conv_layer), axis=-1)
